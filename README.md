...
This will help you understand how to set up GitOrbit to track your own GitHub followers. To get a local copy up and running follow these simple steps.
...
GitOrbit needs two credentials to run — a GitHub token and a MongoDB connection string.
...
3. Set a name like `GitOrbit` and choose an expiration
...
GitOrbit stores all follower snapshots and event history in MongoDB.
...
4. Deploy — copy the URL you receive (e.g. `https://GitOrbit-api.onrender.com`)

### Frontend → Vercel

1. Open `frontend/js/config.js` and set your Render URL:
   ```js
   window.API_BASE = 'https://GitOrbit-api.onrender.com'
   ```
...
To test immediately: Actions tab → **GitOrbit — Nightly Sync** → **Run workflow**.
